...
public final class ErrorPayloadMappersTest {
  ...
  @Test public void clientV2NoError(){
    ...
    Mono.just(responseWithBody).transform(ErrorPayloadMappers.clientV2(this.objectMapper)).as(StepVerifier::create).expectNext(responseWithBody).expectComplete().verify(Duration.ofSeconds(1));
  }
  ...
  @Test public void clientV3ClientError() throws IOException {
    ...
    Mono.just(responseWithBody).transform(ErrorPayloadMappers.clientV3(this.objectMapper)).as(StepVerifier::create).consumeErrorWith(t -> {
      assertThat(t).isInstanceOf(ClientV3Exception.class).hasMessage("CF-UnprocessableEntity(10008): something went wrong").extracting("statusCode").containsExactly(BAD_REQUEST.code());
  ...
  @Test public void clientV3NoError(){
    when(this.response.status()).thenReturn(OK);
    HttpClientResponseWithBody responseWithBody=buildResponseWithBody();
    Mono.just(responseWithBody).transform(ErrorPayloadMappers.clientV3(this.objectMapper)).as(StepVerifier::create).expectNext(responseWithBody).expectComplete().verify(Duration.ofSeconds(1));
  }
  @Test public void clientV3ServerError() throws IOException {
    when(this.response.status()).thenReturn(INTERNAL_SERVER_ERROR);
    HttpClientResponseWithBody responseWithBody=buildResponseWithBody(ByteBufFlux.fromPath(new ClassPathResource("fixtures/client/v3/error_response.json").getFile().toPath()));
    Mono.just(responseWithBody).transform(ErrorPayloadMappers.clientV3(this.objectMapper)).as(StepVerifier::create).consumeErrorWith(t -> {
      assertThat(t).isInstanceOf(ClientV3Exception.class).hasMessage("CF-UnprocessableEntity(10008): something went wrong").extracting("statusCode").containsExactly(INTERNAL_SERVER_ERROR.code());
      assertThat(((ClientV3Exception)t).getErrors()).flatExtracting(org.cloudfoundry.client.v3.Error::getCode,org.cloudfoundry.client.v3.Error::getDetail,org.cloudfoundry.client.v3.Error::getTitle).containsExactly(10008,"something went wrong","CF-UnprocessableEntity");
    }
).verify(Duration.ofSeconds(1));
  }
  ...
  @Test public void uaaNoError(){
    when(this.response.status()).thenReturn(OK);
    HttpClientResponseWithBody responseWithBody=buildResponseWithBody();
    Mono.just(responseWithBody).transform(ErrorPayloadMappers.uaa(this.objectMapper)).as(StepVerifier::create).expectNext(responseWithBody).expectComplete().verify(Duration.ofSeconds(1));
  }
  ...
  public HttpClientResponseWithBody buildResponseWithBody(){
    return buildResponseWithBody(ByteBufFlux.fromInbound(Flux.empty()));
  }
  public HttpClientResponseWithBody buildResponseWithBody(  ByteBufFlux body){
